...
interface
 subroutine psp_dump_outputs(pfx,pspcod,lmnmax,lnmax,mpssoang,&  
  ...
  integer,intent(in) :: n1xccc
  integer,intent(in) :: pspcod
  real(dp),intent(in) :: epsatm
  real(dp),intent(in) :: maxrad
  type(nctab_t),intent(in) :: nctab
  character(len=*), intent(in) :: pfx
  real(dp),intent(in) :: qchrg
  real(dp),intent(in) :: xcccrc
  real(dp),intent(in) :: ekb(lnmax)
  real(dp),intent(in) :: ffspl(mqgrid,2,lnmax)
  integer,intent(in) :: indlmn(6,lmnmax)
  integer,intent(in) :: nproj(mpssoang)
  real(dp),intent(in) :: vlspl(mqgrid,2)
