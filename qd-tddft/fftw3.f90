...
contains
  ...
  subroutine fft_init(n, is_real, fft)
    ...
    if(is_real == fft_real) then
      ...
      call dfftw_plan_dft_2d (fft_array(j)%planf, n(1), n(2), cin, cout, fftw_forward,  fftw_measure)
      call dfftw_plan_dft_2d (fft_array(j)%planb, n(1), n(2), cin, cout, fftw_backward, fftw_measure)
      deallocate(cin, cout)
    end if
    fft = fft_array(j)
